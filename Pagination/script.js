...
function loadPagination(numberOfPages = 1) {
    ...
    if (pages < 2) {
        ...
        prevBtn.addEventListener("click", () => {
            if (currPage > 1) {
                updateChanges(currPage - 1, currPage - 2);
                currPage -= 1;
            }
        })
        nextBtn.addEventListener("click", () => {
            if (currPage < pages) {
                updateChanges(currPage - 1, currPage)
                currPage += 1
            }
        })
        const pageNums = document.querySelectorAll("p");
        ...
        prevBtn.addEventListener("click", () => {
            if (currPage > 1) {
                currPage -= 1
                if (currPage < 4) {
                    updateChanges(currPage, currPage - 1)
        ...
        prevBtn.addEventListener("click", () => {
            if (currPage > 1) {
                ...
                if (currPage < 3) {
                    ...
                    forLastThreeBoxes--;
                    updateChanges(3 + forLastThreeBoxes, 2 + forLastThreeBoxes)
                } else {
        ...
        nextBtn.addEventListener("click", () => {
            if (currPage < pages) {
                if (currPage > pages - 3) {
                    updateChanges(2 + forLastThreeBoxes, 3 + forLastThreeBoxes)
                    changeDotToNumber(3, pages - 1)
                    forLastThreeBoxes++;
                } else if (currPage < 3) {
                    updateChanges((currPage % pageNums.length) - 1, (currPage % pageNums.length))
                } else {
